...
public interface HorariosRepository extends JpaRepository<AgendaHorarioModel, UUID> {
  ...
  @Query(value = """
      SELECT
        ...
        ah.horario_Fim,
        CASE
          WHEN ag.id IS NOT NULL THEN true
          ELSE false
        END AS reservado
      FROM
        Agenda a
      ...
      LEFT JOIN
        Agendamento ag ON ag.horario_id = ah.id AND ag.data_agendamento = :dataAgendamento AND ag.status = 'CONFIRMADO'
      WHERE
  ...
  List<HorarioProjecao> findHorariosByServicoId(@Param("idDoServico") UUID idDoServico,
      @Param("diaSemana") String diaSemana, @Param("dataAgendamento") LocalDate dataAgendamento);
}
